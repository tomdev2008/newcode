...
public class TemplateServiceImpl implements TemplateService,
	...
	public List<Template> getList(TemplateType type) {
		if (type != null)
			try {
				...
				Document localDocument = new SAXReader().read(localFile);
				ArrayList localArrayList = new ArrayList();
				List localList = localDocument
						.selectNodes("/shopxx/template[@type='" + type + "']");
				Iterator localIterator = localList.iterator();
				while (localIterator.hasNext()) {
					Element localElement = (Element) localIterator.next();
					Template localTemplate = servletContext(localElement);
	...
	public Template get(String id) {
		try {
			...
			Element localElement = (Element) localDocument
					.selectSingleNode("/shopxx/template[@id='" + id + "']");
			Template localTemplate = servletContext(localElement);
	...
	public String read(Template template) {
		String str1 = this.servletContext.getRealPath(this.IIIllIll[0]
	...
	public void write(Template template, String content) {
		String str = this.servletContext.getRealPath(this.IIIllIll[0]
				+ template.getTemplatePath());
