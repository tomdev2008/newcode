...
public class SearchServiceImpl implements SearchService {
	...
	public void index(Class<?> type) {
		...
		if (type == Article.class)
			for (i = 0; i < this.articleDao.count(new Filter[0]); i += 20) {
				localList = this.articleDao.findList(Integer.valueOf(i),
			...
			for (i = 0; i < this.productDao.count(new Filter[0]); i += 20) {
				localList = this.productDao.findList(Integer.valueOf(i),
						Integer.valueOf(20), null, null);
				localIterator = localList.iterator();
				while (localIterator.hasNext()) {
	...
	public Page<Article> search(String keyword, Pageable pageable) {
		...
		try {
			...
			return new Page(localFullTextQuery.getResultList(),
	...
	public Page<Product> search(String keyword, BigDecimal startPrice,
		...
		try {
			...
			if ((startPrice != null)
				...
				localObject = NumericRangeQuery.newDoubleRange("price",
				...
				localBooleanQuery2.add((Query) localObject,
					...
					&& (endPrice.compareTo(new BigDecimal(0)) >= 0)) {
				localObject = NumericRangeQuery.newDoubleRange("price", null,
						Double.valueOf(endPrice.doubleValue()), false, true);
				localBooleanQuery2.add((Query) localObject,
						BooleanClause.Occur.MUST);
			}
