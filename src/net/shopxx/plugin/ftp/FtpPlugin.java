...
public class FtpPlugin extends StoragePlugin {
	...
	public void upload(String path, File file, String contentType) {
		...
		if (localPluginConfig != null) {
			...
			Integer localInteger = Integer.valueOf(localPluginConfig
					.getAttribute("port"));
			...
			try {
				...
				if (FTPReply
						.isPositiveCompletion(localFTPClient.getReplyCode())) {
	...
	public List<FileInfo> browser(String path) {
		...
		if (localPluginConfig != null) {
			...
			try {
				...
				localFTPClient.setFileType(2);
				localFTPClient.enterLocalPassiveMode();
				if ((FTPReply.isPositiveCompletion(localFTPClient
						.getReplyCode()))
						&& (localFTPClient.changeWorkingDirectory(path)))
					for (FTPFile localFTPFile : localFTPClient.listFiles()) {
						FileInfo localFileInfo = new FileInfo();
						localFileInfo.setName(localFTPFile.getName());
						localFileInfo.setUrl(str4 + path
								+ localFTPFile.getName());
						localFileInfo.setIsDirectory(Boolean
								.valueOf(localFTPFile.isDirectory()));
						localFileInfo.setSize(Long.valueOf(localFTPFile
								.getSize()));
						localFileInfo.setLastModified(localFTPFile
