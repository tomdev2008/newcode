...
public class StaticController extends BaseController {
	...
	@ResponseBody
	public Map<String, Object> build(StaticControllerBuildType buildType,
		...
		if (buildType == StaticControllerBuildType.index) {
			...
			if (buildType == StaticControllerBuildType.article) {
				localObject1 = (ArticleCategory) this.articleCategoryService
						.find(articleCategoryId);
				localList = this.articleService.findList(
						(ArticleCategory) localObject1, beginDate, endDate,
						first, count);
				...
				localObject1 = (ProductCategory) this.productCategoryService
						.find(productCategoryId);
				localList = this.productService.findList(
						(ProductCategory) localObject1, beginDate, endDate,
						first, count);
				localIterator = localList.iterator();
				while (localIterator.hasNext()) {
