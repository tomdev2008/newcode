	...
	public String view(SalesControllerType type, Date beginDate, Date endDate,
		...
		for (int n = i; n <= j; n++) {
			...
			if (type == SalesControllerType.year) {
				...
				localObject1 = this.orderService.getSalesAmount(localDate1,
						localDate2);
				localObject2 = this.orderService.getSalesVolume(localDate1,
						localDate2);
				localLinkedHashMap1.put(localDate1,
						localObject1 != null ? localObject1 : BigDecimal.ZERO);
				localLinkedHashMap2
						.put(localDate1,
								Integer.valueOf(localObject2 != null ? ((Integer) localObject2)
										.intValue() : 0));
			} else {
				for (int i1 = n == i ? k : localCalendar3.getActualMinimum(2); i1 <= (n == j ? m
						: localCalendar3.getActualMaximum(2)); i1++) {
					if (localLinkedHashMap1.size() >= 12)
						break;
					...
					localObject2 = this.orderService.getSalesAmount(localDate2,
							(Date) localObject1);
					Integer localInteger = this.orderService.getSalesVolume(
							localDate2, (Date) localObject1);
					localLinkedHashMap1.put(localDate2,
							localObject2 != null ? localObject2
									: BigDecimal.ZERO);
					localLinkedHashMap2.put(localDate2, Integer
							.valueOf(localInteger != null ? localInteger
									.intValue() : 0));
				}
			}
		}
