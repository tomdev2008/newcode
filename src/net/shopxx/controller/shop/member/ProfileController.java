...
public class ProfileController extends BaseController {
	...
	public String update(String email, HttpServletRequest request,
		...
		if ((!localSetting.getIsDuplicateEmail().booleanValue())
				&& (!this.IIIlllIl.emailUnique(localMember.getEmail(), email)))
			return "/shop/common/error";
		...
		while (localIterator.hasNext()) {
			...
			if ((localMemberAttribute.getType() == MemberAttributeType.name)
					|| (localMemberAttribute.getType() == MemberAttributeType.address)
					|| (localMemberAttribute.getType() == MemberAttributeType.zipCode)
					|| (localMemberAttribute.getType() == MemberAttributeType.phone)
					|| (localMemberAttribute.getType() == MemberAttributeType.mobile)
					|| (localMemberAttribute.getType() == MemberAttributeType.text)
					|| (localMemberAttribute.getType() == MemberAttributeType.select)) {
				...
				if (localMemberAttribute.getType() == MemberAttributeType.gender) {
					localGender = StringUtils.isNotEmpty(str) ? MemberGender
							.valueOf(str) : null;
					if ((localMemberAttribute.getIsRequired().booleanValue())
							&& (localGender == null))
						return "/shop/common/error";
					...
					try {
						localGender = StringUtils.isNotEmpty(str) ? DateUtils
					...
					if (localMemberAttribute.getType() == MemberAttribute.Type.area) {
						localObject1 = StringUtils.isNotEmpty(str) ? (Area) this.IIIlllll
						...
						localObject1 = request
								.getParameterValues("memberAttribute_"
										+ localMemberAttribute.getId());
						Object localObject2 = localObject1 != null ? Arrays
						...
						if ((localMemberAttribute.getIsRequired()
								...
								&& ((localObject2 == null) || (localObject2
										.isEmpty())))
							return "/shop/common/error";
						localMember.setAttributeValue(localMemberAttribute,
								localObject2);
					}
				}
			}
