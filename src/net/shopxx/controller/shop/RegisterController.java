...
public class RegisterController extends BaseController {
	...
	public Message submit(String captchaId, String captcha, String username,
		...
		if (!this.IIIlllIl.isValid(Setting.CaptchaType.memberRegister,
				captchaId, captcha))
		...
		if (IIIllIlI(Member.class, "username", username,
				new Class[] { BaseEntity.class }))
			if (IIIllIlI(Member.class, "password", str,
					new Class[] { BaseEntity.class }))
				if (IIIllIlI(Member.class, "email", email,
						new Class[] { BaseEntity.class }))
					break label154;
		return Message.error("shop.common.invalid", new Object[0]);
		label154: if ((username.length() < localSetting.getUsernameMinLength()
				.intValue())
		...
		while (((Iterator) localObject2).hasNext()) {
			...
			localObject3 = request.getParameter("memberAttribute_"
					+ ((MemberAttribute) localObject1).getId());
			if ((((MemberAttribute) localObject1).getType() == MemberAttribute.Type.name)
				...
				if ((((MemberAttribute) localObject1).getIsRequired()
						.booleanValue())
						&& (StringUtils.isEmpty((String) localObject3)))
					return Message.error("shop.common.invalid", new Object[0]);
				localMember.setAttributeValue((MemberAttribute) localObject1,
						localObject3);
			} else {
				Member.Gender localGender;
				if (((MemberAttribute) localObject1).getType() == MemberAttribute.Type.gender) {
					localGender = StringUtils.isNotEmpty((String) localObject3) ? Member.Gender
							.valueOf((String) localObject3) : null;
					if ((((MemberAttribute) localObject1).getIsRequired()
					...
					try {
						localGender = StringUtils
								.isNotEmpty((String) localObject3) ? DateUtils
								.parseDate((String) localObject3,
					...
					localObject4 = StringUtils
							.isNotEmpty((String) localObject3) ? (Area) this.IIlIIIlI
							.find(Long.valueOf((String) localObject3)) : null;
					...
					if (((MemberAttribute) localObject1).getType() != MemberAttribute.Type.checkbox)
						continue;
					localObject4 = request
							.getParameterValues("memberAttribute_"
									+ ((MemberAttribute) localObject1).getId());
					localObject5 = localObject4 != null ? Arrays
							.asList(localObject4) : null;
					if ((((MemberAttribute) localObject1).getIsRequired()
							.booleanValue())
							&& ((localObject5 == null) || (((List) localObject5)
									.isEmpty())))
						return Message.error("shop.common.invalid",
								new Object[0]);
					localMember.setAttributeValue(
							(MemberAttribute) localObject1, localObject5);
				}
			}
		}
		...
		while (((Enumeration) localObject3).hasMoreElements()) {
			...
			((Map) localObject2).put(localObject4,
					session.getAttribute((String) localObject4));
		}
		session.invalidate();
